...
The trainee Fiscal Year Credit Score page shows the current attendance snapshot, for example `36 / 90`, eligible awards, claimed history, and total claimed credits. Credit-bearing course cards and completed catalog course details show the configured course credit value before completion and expose the claim action once the completion award is eligible; the assessment result and completed-course summary repeat the amount and claim prompt; completed enrollments from before credit awards were introduced can recover and claim their missing idempotent course award; claimed awards are shown as claimed. Attendance is refreshed through an `AttendanceProviderInterface`; the default local implementation is a sandbox provider configured through `TMIS_SANDBOX_PRESENT_DAYS`. A future TMIS REST provider can replace the adapter without changing the credit ledger.
