...
The trainee portal uses a focused top navigation instead of the authenticated sidebar. Its tabs are **Course**, **My Courses**, **Tests**, **My Tests**, and a disabled **Feedback** placeholder. Course opens the overview and links to the catalog; My Courses shows active and completed enrollments; Tests shows assigned tests that have not been started; and My Tests shows every assigned test with its current attempt state, actions, and results. Feedback has no route or workflow yet and is intentionally non-interactive.
