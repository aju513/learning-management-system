...
Staff course index cards use a responsive 3:2 thumbnail presentation (up to 600px wide), with a subtle lift/shadow and image zoom on hover. The course title changes to the brand color on hover and acts as a pointer target; authorized staff can activate the whole card with a click or Enter key, while the permission-protected Open action remains available in the footer. Staff cards show the course status, title, and a footer containing the created date and instructor on the left; descriptions and module/trainee counts are intentionally omitted. Trainee catalog cards remain a separate presentation with catalog-specific metadata and application actions.
