...
<div class="space-y-4">
    ...
    <div class="grid gap-4 sm:grid-cols-2">
        <x-form.select name="type" label="Question type" :options="collect(\App\Enums\CourseAssessmentQuestionType::cases())->mapWithKeys(fn ($type) => [$type->value => $type->label()])->all()" :value="$question?->type?->value ?? 'single_choice'" required />
        ...
        <div class="space-y-2">
            @foreach($existingOptions as $index => $option)
                <div class="grid grid-cols-[auto_1fr] items-center gap-3">
                    ...
                    <x-form.input :id="'course-option-'.$index.'-'.($question?->id ?? 'new')" name="options[]" :value="$option" :label="'Option '.($index + 1)" required />
                </div>
            @endforeach
        </div>
    </fieldset>
