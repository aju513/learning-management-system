...
<x-common.page-breadcrumb :pageTitle="$assessment->material->title"><x-slot:actions>
    <a href="{{ route(\App\Support\PortalRoute::name('courses.show'), $assessment->material->chapter->module->course) }}#chapter-{{ $assessment->material->chapter_id }}" class="rounded-lg border border-gray-300 px-4 py-2.5 text-sm dark:border-gray-700 dark:text-white">Back to course</a>
...
<div class="grid gap-6 xl:grid-cols-[1fr_320px]">
    <div class="space-y-6">
        ...
        @if($assessment->attempts_count === 0)
            <x-common.component-card title="Add question" desc="Use multiple choice when more than one answer is correct.">
    ...
    <aside><x-common.component-card title="Assessment settings"><dl class="space-y-3 text-sm"><div class="flex justify-between"><dt class="text-gray-500">Passing score</dt><dd class="font-medium text-gray-800 dark:text-white">{{ $assessment->passing_percentage }}%</dd></div><div class="flex justify-between"><dt class="text-gray-500">Questions</dt><dd class="font-medium text-gray-800 dark:text-white">{{ $assessment->questions->count() }}</dd></div><div class="flex justify-between"><dt class="text-gray-500">Attempts</dt><dd class="font-medium text-gray-800 dark:text-white">Unlimited</dd></div><div class="flex justify-between"><dt class="text-gray-500">Started attempts</dt><dd class="font-medium text-gray-800 dark:text-white">{{ $assessment->attempts_count }}</dd></div></dl><p class="mt-4 border-t border-gray-100 pt-4 text-sm text-gray-500 dark:border-gray-800">After submission, trainees see their selected and correct answers. A passing result completes this learning material.</p></x-common.component-card></aside>
</div>
@endsection
