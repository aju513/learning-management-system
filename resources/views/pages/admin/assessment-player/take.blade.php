...
<div x-data="{
    ...
    <x-common.page-breadcrumb :pageTitle="$attempt->assessment->title" :translate="false">
        <x-slot:actions><div class="rounded-lg bg-warning-50 px-4 py-2 text-sm font-semibold text-warning-700 dark:bg-warning-500/10" aria-live="polite"><span x-text="`${Math.floor(remaining / 60)}:${String(remaining % 60).padStart(2, '0')}`">--:--</span> remaining</div></x-slot:actions>
    ...
    <x-common.component-card :title="'Attempt '.$attempt->attempt_number" :desc="$attempt->assessment->instructions ?? 'Answer every question, then submit for grading.'">
        ...
        <form method="POST" action="{{ route('learning.assessments.attempts.submit', $attempt) }}" class="space-y-8" @submit.prevent="submitAssessment()" @change="saveDraft" @input.debounce.700ms="saveDraft">
            ...
            @endforeach
            <div class="flex flex-wrap items-center justify-between gap-3"><div><p class="text-xs text-gray-500" x-text="saveState">Saved answers loaded</p><p x-show="submitError" x-text="submitError" class="mt-1 text-sm text-error-600" role="alert"></p></div><button type="submit" aria-label="Submit assessment" class="inline-flex items-center gap-2 rounded-lg bg-brand-500 px-6 py-3 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-60" :disabled="submitting"><i class="bi bi-check2-circle" aria-hidden="true"></i><span x-text="submitting ? 'Submitting...' : 'Submit assessment'">Submit assessment</span></button></div>
        </form>
