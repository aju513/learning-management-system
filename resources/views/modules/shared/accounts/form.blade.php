...
<x-common.component-card :title="$title" :desc="'This account will use the fixed '.$role->label().' portal and permissions.'">
    <form method="POST" action="{{ $user->exists ? route($routeBase.'.update', $user) : route($routeBase.'.store') }}" class="space-y-5">
        @csrf
        ...
        <div class="flex justify-end gap-3"><a href="{{ route($routeBase.'.index') }}" class="rounded-lg border border-gray-300 px-4 py-2.5 text-sm dark:border-gray-700 dark:text-white">Cancel</a><button class="rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white">{{ $user->exists ? 'Save changes' : 'Create account' }}</button></div>
    </form>
