    ...
    <x-common.page-breadcrumb :pageTitle="$title">
        <x-slot:actions>
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('learning.catalog.index') }}" class="rounded-lg border border-gray-300 px-4 py-2.5 text-sm font-medium text-gray-700 dark:border-gray-700 dark:text-gray-200">{{ __('Browse courses') }}</a>
                <a href="{{ route('learning.assessments.index') }}" class="rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white">{{ __('View tests') }}</a>
            </div>
        </x-slot:actions>
    </x-common.page-breadcrumb>

    <div class="mb-6 rounded-2xl bg-gradient-to-r from-brand-500 to-brand-700 p-6 text-white">
        <p class="text-sm text-white/75">{{ __($context) }}</p>
        <h1 class="mt-1 text-2xl font-bold">{{ __('Welcome back, ') }}{{ auth()->user()->name }}</h1>
    ...
    <div class="mt-6 space-y-6">
            ...
            <div class="space-y-7">
                @forelse ($courseGroups as $trainingKey => $courses)
                    <section>
                        <div class="mb-3 flex items-center justify-between gap-3">
                            <div>
                                <p class="text-xs font-semibold uppercase tracking-wide text-brand-500">{{ __('Training access') }}</p>
                                <h2 class="mt-1 text-lg font-semibold text-gray-800 dark:text-white">{{ $trainingTitle($trainingKey) }}</h2>
                            </div>
                            <x-ui.badge color="light">{{ $courses->count() }} {{ __($courses->count() === 1 ? 'course' : 'courses') }}</x-ui.badge>
                        </div>
                            ...
                            @foreach ($courses as $course)
                                ...
                                <article class="rounded-xl border border-gray-200 p-4 dark:border-gray-800">
                                    ...
                                    <div class="mt-4 flex items-center justify-between gap-3">
                                        <a href="{{ route('learning.catalog.show', $course) }}" class="text-sm font-medium text-brand-500">{{ __('View course') }}</a>
                                        ...
                                        @else
                                            <span class="text-xs text-gray-400">{{ __('Apply from details') }}</span>
                                        @endif
                                    </div>
                                </article>
                            @endforeach
                ...
                @empty
                    <div class="rounded-xl border border-dashed border-gray-300 p-10 text-center dark:border-gray-700">
                        <h2 class="font-semibold text-gray-800 dark:text-white">{{ __('No courses available yet') }}</h2>
        ...
        <x-common.component-card title="Available tests" desc="Assigned tests are grouped by the training enrollment that makes them available.">
            <div class="space-y-7">
                @forelse ($testGroups as $trainingKey => $tests)
                    <section>
                        <div class="mb-3 flex items-center justify-between gap-3">
                            ...
                            <x-ui.badge color="light">{{ $tests->count() }} {{ __($tests->count() === 1 ? 'test' : 'tests') }}</x-ui.badge>
                        </div>
                        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                            @foreach ($tests as $test)
                                <article class="rounded-xl border border-gray-200 p-4 dark:border-gray-800">
                                    <div class="flex items-start justify-between gap-3">
                                        ...
                                        <x-ui.badge color="success">{{ __('Available') }}</x-ui.badge>
                                    </div>
                                    ...
                                    <form method="POST" action="{{ route('learning.assessments.start', $test) }}" class="mt-4">@csrf<button class="w-full rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white">{{ __('Start / continue test') }}</button></form>
                                </article>
                            @endforeach
                        </div>
                    </section>
                @empty
                    <div class="rounded-xl border border-dashed border-gray-300 p-10 text-center dark:border-gray-700">
                        <h2 class="font-semibold text-gray-800 dark:text-white">{{ __('No tests available yet') }}</h2>
                        <p class="mt-1 text-sm text-gray-500">{{ __('Assigned tests will appear here when they are published and available to your training enrollment.') }}</p>
                    </div>
                @endforelse
            </div>
        </x-common.component-card>
