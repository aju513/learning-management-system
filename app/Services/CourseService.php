...
class CourseService
{
    ...
    public function changeStatus(Course $course, CourseStatus $status, User $actor): Course
    {
        if ($status === CourseStatus::Published) {
            ...
            if ($course->modules->isEmpty() || $course->modules->every(fn (CourseModule $module) => $module->materials->isEmpty())) {
                throw ValidationException::withMessages(['status' => 'Add at least one module with learning material before publishing.']);
            }
        }
    ...
    public function deleteCourse(Course $course, User $actor): void
    {
        ...
        $course = $this->courses->findCourseDetails($course);
        $materialFiles = $course->modules->flatMap->materials->pluck('file_path')->filter();
    ...
    public function deleteModule(CourseModule $module, User $actor): void
    {
        ...
        $files = $module->materials->pluck('file_path')->filter();
    ...
    public function createMaterial(CourseModule $module, array $data, User $actor): LearningMaterial
    {
        $data = $this->prepareMaterialData($data, $actor);
        $material = $this->courses->createMaterial([...$data, 'course_module_id' => $module->id, 'position' => $this->courses->nextMaterialPosition($module)]);
        activity('lms')->causedBy($actor)->performedOn($material)->event('learning-material.created')
            ->withProperties(['course_id' => $module->course_id, 'type' => $material->type->value])->log('Learning material created');
    ...
    public function updateMaterial(LearningMaterial $material, array $data, User $actor): LearningMaterial
    {
        ...
        if (array_key_exists('file_path', $data) && $oldFile) {
